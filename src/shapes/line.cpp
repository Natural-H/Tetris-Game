...
void Line::update()
{
    bool newStateR = rightButton.isPressed();
    bool newStateL = leftButton.isPressed();
    ...
    if (active)
    {
        if (newStateR != lastR && newStateR)
            moveRight();
        if (newStateL != lastL && newStateL)
            moveLeft();
    }
...
void Line::moveLeft()
{
    dots[1].setX(dots[1].getX() - 1);
    dots[0].setX(dots[1].getX() - 1);
}
